...
export default function ValidadorSunat({ isOpen, onClose }: ValidadorSunatProps) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(`/api/search-document?document=${encodeURIComponent(q)}`, {
      ...
      // Preferimos _raw (todos los campos SUNAT); si no existe, mostramos el json
  ...
  return (
    <Drawer isOpen={isOpen} onClose={handleClose} title="🔍 Validador de Razón Social" width="w-[42%]">
        ...
        <div className="space-y-2">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={loading || !/^\d{11}$/.test(query.trim())}
