  ...
  const searchResults = () => {
    ...
    contacts.forEach((c) => {
      if (
        ...
        const org = organizations.find((o) => o.id === c.organizacionId);
        results.push({
          type: 'contact',
    ...
    leads.forEach((l) => {
      ...
      if (
        ...
        results.push({
          type: 'lead',
          label: `${l.id} · ${org?.nombre ?? '—'}`,
          sub: `${l.estado.replace('_', ' ').toUpperCase()} · ${l.servicioInteres ?? '—'}`,
          href: `/pipeline`,
        });
      }
    });
    ...
    quotes.forEach((qt) => {
      if (
        ...
        qt.servicio.toLowerCase().includes(q) ||
        qt.dirigidoA.toLowerCase().includes(q) ||
        qt.producto?.toLowerCase().includes(q) ||
        qt.remitente.toLowerCase().includes(q)
      ) {
        results.push({
          type: 'quote',
          label: `${qt.id} · ${qt.cliente}`,
          sub: `${qt.servicio} · ${qt.estado.toUpperCase()}`,
          href: `/quotes/${qt.id}/print`,
        });
      }
    });
